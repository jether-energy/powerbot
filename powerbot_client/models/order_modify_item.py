...
class OrderModifyItem(BaseModel):
    """
    OrderModifyItem
    """ # noqa: E501
    order_id: StrictStr
    revision_no: Optional[StrictInt] = Field(default=None, description="The revision of the order that should be modified. If the exchange already has a newer revision, the modification will be rejected.  This field is optional if the order action is `DELE` or `DEAC`. In this case, the system will try to set the last known revision number.")
    changes: OrderModify
    __properties: ClassVar[List[str]] = ["order_id", "revision_no", "changes"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "changes": OrderModify.from_dict(obj["changes"]) if obj.get("changes") is not None else None
        })
        return _obj
