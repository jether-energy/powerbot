...
class Orders(BaseModel):
    ...
    delta: Optional[StrictBool] = Field(default=None, description="if true, the underlying bid / asks need to be interpreted as delta to the previous revision, if false the bids/ask represent a current snapshot.")
    bid: Optional[List[OrderBookEntry]] = None
    ask: Optional[List[OrderBookEntry]] = None
    __properties: ClassVar[List[str]] = ["contract_id", "product", "delivery_area", "sequence_number", "revision_no", "delta", "bid", "ask"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in bid (list)
        _items = []
        if self.bid:
            for _item in self.bid:
                if _item:
                    _items.append(_item.to_dict())
            _dict['bid'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in ask (list)
        _items = []
        if self.ask:
            for _item in self.ask:
                if _item:
                    _items.append(_item.to_dict())
            _dict['ask'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "delta": obj.get("delta"),
            "bid": [OrderBookEntry.from_dict(_item) for _item in obj["bid"]] if obj.get("bid") is not None else None,
            "ask": [OrderBookEntry.from_dict(_item) for _item in obj["ask"]] if obj.get("ask") is not None else None
        })
        return _obj
