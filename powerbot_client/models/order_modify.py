...
class OrderModify(BaseModel):
    ...
    OrderModify
    """ # noqa: E501
    action: StrictStr = Field(description="* ACTI: Activates the order. Ignored if already active. * DEAC: Deactivates (hibernates) the order. Hibernated orders are removed from the order book but are still available for modification or activation in the own orders list. * MODI: Modify the order * DELE: Delete the order")
    validity_res: Optional[ValidityRestriction] = Field(default=None, alias="validityRes")
    validity_date: Optional[datetime] = Field(default=None, description="Set this field, if you want to update the validity date of the order. Only mandatory in case of validityRes equals \"GTD\". It is used to define the date until which the order is valid. The remaining part of the order will be removed from the order book after this point in time.", alias="validityDate")
    type: Optional[OrderType] = None
    txt: Optional[StrictStr] = Field(default=None, description="Set this field if you want to update the order's 'custom text'")
    ...
    quantity: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Set this field if you want to update the order's quantity.")
    display_qty: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Set this field if you want to update the display quantity of an iceberg Order. This field is required only in the case of type=\"I\".", alias="displayQty")
    price: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Set this field if you want to update the order's description.")
    ppd: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Set this field if you want to update the Peak price delta for iceberg orders. * The ppd of buy orders must be smaller or equal than zero. * The ppd of sell orders must be greater or equal than zero. If it is omitted the system will assume a value of `0.00`.")
    ...
    __properties: ClassVar[List[str]] = ["action", "validityRes", "validityDate", "type", "txt", "ordrExeRestriction", "quantity", "displayQty", "price", "ppd", "expectedNetPos", "expectedMinNetPos", "expectedMaxNetPos", "expectedAbsPos", "expectedSellAbsPos", "expectedBuyAbsPos", "location"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACTI', 'DEAC', 'MODI', 'DELE']):
            raise ValueError("must be one of enum values ('ACTI', 'DEAC', 'MODI', 'DELE')")
        return value
