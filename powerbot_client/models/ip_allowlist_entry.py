...
class IPAllowlistEntry(BaseModel):
    ...
    IPAllowlistEntry
    """ # noqa: E501
    address: Annotated[str, Field(strict=True)] = Field(description="IP or IP/mask")
    description: Optional[Annotated[str, Field(strict=True, max_length=512)]] = Field(default=None, description="description for the IP address")
    __properties: ClassVar[List[str]] = ["address", "description"]

    @field_validator('address')
    def address_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])(\.([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])){3}(\/([12][0-9]?|3[0-2]))?$", value):
            raise ValueError(r"must validate the regular expression /^([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])(\.([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])){3}(\/([12][0-9]?|3[0-2]))?$/")
        return value
