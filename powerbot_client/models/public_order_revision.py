...
class PublicOrderRevision(BaseModel):
    """
    PublicOrderRevision
    """ # noqa: E501
    order_id: StrictStr = Field(description="The unique ID of this order.")
    price: Union[StrictFloat, StrictInt]
    quantity: Union[StrictFloat, StrictInt] = Field(description="The quantity of the orders. A value of zero means that the order was deleted, hibernated or fully executed.")
    created_at: datetime = Field(description="The time when the order first appeared.")
    updated_at: datetime = Field(description="The time when the order was last modified. This can be earlier than the `time` field of the order book revision because the exchange batches multiple order changes into one revision.")
    __properties: ClassVar[List[str]] = ["order_id", "price", "quantity", "created_at", "updated_at"]
