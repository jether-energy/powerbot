...
class AlgoInstanceParameters(BaseModel):
    """
    AlgoInstanceParameters
    """ # noqa: E501
    instance_id: Annotated[str, Field(strict=True)] = Field(description="Unique id which needs to be provided to identify the algorithm instance.")
    tenant_id: Optional[StrictStr] = Field(default=None, description="master-API-key needs to specify a tenant on creation. For tenant-master-API-keys this field is optional, as the tenant is already specified via the key.")
    portfolio_ids: Optional[List[StrictStr]] = Field(default=None, description="Specify the portfolios the algorithm should run in.")
    parameters: Optional[Dict[str, Any]] = Field(default=None, description="A freely defined JSON object with key/value pairs.")
    resources: Optional[Resources] = None
    __properties: ClassVar[List[str]] = ["instance_id", "tenant_id", "portfolio_ids", "parameters", "resources"]

    @field_validator('instance_id')
    def instance_id_validate_regular_expression(cls, value):
