...
class CashLimit(BaseModel):
    ...
    limit: Union[Annotated[float, Field(multiple_of=0.01, strict=True)], Annotated[int, Field(strict=True)]] = Field(description="The limit for the currency. Calculated over all contracts per day (converted to local time).")
    exchange: Exchange
    __properties: ClassVar[List[str]] = ["direction", "currency", "limit", "exchange"]

    @field_validator('direction')
    def direction_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NEG', 'POS']):
            raise ValueError("must be one of enum values ('NEG', 'POS')")
        return value
